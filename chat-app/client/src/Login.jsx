...
export default function Login({ onSwitch }) {
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', fontFamily:"'DM Sans',sans-serif", position:'relative', overflow:'hidden', background:'#07070f' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 48px', position:'relative', zIndex:1 }}>
        ...
        <div style={{ position:'relative', width:160, height:160, display:'flex', alignItems:'center', justifyContent:'center', marginBottom:44 }}>
          ...
          <div style={{
            ...
            boxShadow:'0 0 50px rgba(79,70,229,.5)',
          }}>💬</div>
        </div>

        <h1 style={{
          fontFamily:"'Syne',sans-serif", fontSize:58, fontWeight:800,
          letterSpacing:'-2.5px', lineHeight:1, marginBottom:16, textAlign:'center',
          background:'linear-gradient(135deg,#e0e7ff 10%,#a5b4fc 45%,#f472b6 80%)',
        ...
        {[
          ...
          <div key={i} style={{
            ...
            padding:'12px 18px', borderRadius:14, marginBottom:8,
            background:'rgba(255,255,255,0.03)',
            border:'1px solid rgba(255,255,255,0.06)',
            width:'100%', maxWidth:320,
            animation:`fadeSlideUp .6s ${.6+i*.1}s both`, opacity:0,
          }}>
      ...
      {/* Vertical divider */}
      <div style={{ width:1, background:'linear-gradient(to bottom,transparent,rgba(255,255,255,0.07) 20%,rgba(255,255,255,0.07) 80%,transparent)', flexShrink:0, position:'relative', zIndex:1 }}/>

      {/* Right — form */}
      <div style={{ width:500, display:'flex', alignItems:'center', justifyContent:'center', padding:'48px 40px', position:'relative', zIndex:1 }}>
        <div style={{
          ...
          <div style={{ position:'relative', zIndex:1 }}>
            ...
            <h2 style={{ fontSize:30, fontWeight:800, color:'#fff', letterSpacing:'-0.5px', margin:'0 0 8px', fontFamily:"'Syne',sans-serif" }}>Welcome back</h2>
            <p style={{ color:'rgba(255,255,255,0.35)', fontSize:13, marginBottom:32 }}>Sign in to re-enter the cosmos</p>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <div style={{ position:'relative' }}>
                ...
                <input placeholder="you@cosmos.io" value={form.email} onChange={e=>update('email',e.target.value)} onFocus={()=>setFocused('email')} onBlur={()=>setFocused(null)} style={inp(focused,'email')}/>
              </div>
            </div>
            ...
            <div style={{ marginBottom:8 }}>
              ...
              <div style={{ position:'relative' }}>
                <span style={{ position:'absolute', left:14, top:'50%', transform:'translateY(-50%)', fontSize:16, opacity:.5 }}>🔒</span>
                <input type={showPass?'text':'password'} placeholder="••••••••" value={form.password} onChange={e=>update('password',e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleLogin()} onFocus={()=>setFocused('password')} onBlur={()=>setFocused(null)} style={{...inp(focused,'password'), paddingRight:44}}/>
            ...
            {resetSent && <div style={{ background:'rgba(52,211,153,0.1)', border:'1px solid rgba(52,211,153,0.25)', borderRadius:12, padding:'10px 14px', marginBottom:16, color:'#6ee7b7', fontSize:13 }}>✓ Reset email sent! Check your inbox.</div>}

            {error && <div style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:12, padding:'10px 14px', marginBottom:16, color:'#fca5a5', fontSize:13, display:'flex', gap:8 }}>⚠ {error}</div>}
            ...
            <p style={{ textAlign:'center', marginTop:22, fontSize:13, color:'rgba(255,255,255,.35)' }}>
              New to NexChat?{' '}
